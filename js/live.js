const liveTab = (() => {
  ...
  function renderActive(drill) {
    ...
    btWarn.textContent = '블루투스를 확인하세요';
    btWarn.style.display = bluetooth.isConnected() ? 'none' : '';
    ...
    // ── IMU yaw widget ────────────────────────────────────
    const imuCard = document.createElement('div');
    imuCard.className = 'card card-sm';
    imuCard.id = 'imu-card';
    imuCard.style.cssText = 'display:flex;align-items:center;gap:var(--gap-md);padding:var(--gap-sm) var(--gap-md);';

    // Foot icon that rotates with yaw delta
    const imuIcon = document.createElement('div');
    imuIcon.id = 'imu-icon';
    imuIcon.style.cssText = 'font-size:22px;transition:transform .15s ease;';
    imuIcon.textContent = '🦶';

    const imuInfo = document.createElement('div');
    imuInfo.style.cssText = 'flex:1;display:flex;flex-direction:column;gap:2px;';

    const imuLabel = document.createElement('span');
    imuLabel.style.cssText = 'font-size:10px;letter-spacing:.08em;color:var(--text-muted);';
    imuLabel.textContent = 'YAW OFFSET';

    const imuVal = document.createElement('span');
    imuVal.id = 'imu-yaw-val';
    imuVal.style.cssText = 'font-family:var(--font-mono);font-size:16px;font-weight:700;';
    imuVal.textContent = '—';

    const imuTol = document.createElement('span');
    imuTol.id = 'imu-tol-label';
    imuTol.style.cssText = 'font-size:10px;color:var(--text-dim);';
    imuTol.textContent = `허용 ±${drill.yawTolerance ?? 10}°`;

    imuInfo.appendChild(imuLabel);
    imuInfo.appendChild(imuVal);
    imuInfo.appendChild(imuTol);

    const btnCal = document.createElement('button');
    btnCal.className = 'btn btn-ghost';
    btnCal.id = 'btn-yaw-cal';
    btnCal.style.cssText = 'font-size:10px;padding:4px 8px;';
    btnCal.textContent = '⊕ YAW CAL';
    btnCal.title = '현재 발 방향을 기준 0°로 설정';
    btnCal.onclick = () => {
      const ref = session.calibrateYaw();
      btnCal.textContent = '✓ CALIBRATED';
      setTimeout(() => { btnCal.textContent = '⊕ YAW CAL'; }, 1500);
    };

    imuCard.appendChild(imuIcon);
    imuCard.appendChild(imuInfo);
    imuCard.appendChild(btnCal);
    panel.appendChild(imuCard);
  ...
  function updateImu(imu, alerts) {
    const icon  = document.getElementById('imu-icon');
    const valEl = document.getElementById('imu-yaw-val');
    const card  = document.getElementById('imu-card');
    if (!icon || !valEl || !card) return;

    const ref   = session.yawRef;
    if (ref === null) { valEl.textContent = '—'; return; }

    let delta = imu.yaw - ref;
    if (delta > 180) delta -= 360;
    if (delta < -180) delta += 360;
    delta = Math.round(delta * 10) / 10;

    const hasImu = alerts.some(a => a.type === 'imu');
    icon.style.transform  = `rotate(${delta}deg)`;
    valEl.textContent     = (delta >= 0 ? '+' : '') + delta + '°';
    valEl.style.color     = hasImu ? 'var(--color-alert-p)' : 'var(--color-ok)';
    card.style.borderColor = hasImu ? 'var(--color-alert-p)' : '';
  }
  ...
  function updateBanner(alerts) {
    ...
    const msgs = alerts.map(a => {
      ...
      if (a.type === 'imu')  return `발 틀어짐 ${a.value > 0 ? '+' : ''}${a.value}° (허용 ±${a.thr}°)`;
    ...
    const hasImu  = alerts.some(a => a.type === 'imu');
    banner.className = 'alert-banner visible ' + (hasGait || hasNeg || hasImu ? 'negative' : 'positive');
  }
  ...
  return {
    ...
    startSession(drill) {
      ...
      session.onValues = (values, alerts, accuracy) => {
        ...
        updateBanner(alerts);
        updateImu(session.currentImu, alerts);
      ...
      // Show BT warning if not connected
      const btWarn = document.getElementById('bt-warn');
      if (btWarn) btWarn.style.display = bluetooth.isConnected() ? 'none' : '';

      if (!bluetooth.isConnected()) return;
      // (simulation removed — real BT only)
    },
  };
})();
